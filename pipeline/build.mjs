...
// transport into OpenStreetMap (2022–2024, DT4A Innovation Challenge funding)
...
//     "(Minibus)" at the end of route_long_name. They ride the "metroline"
//     machinery this family built for GZM and reused for Cairo's paratransit:
//     their own colour, their own toggle, dashed where they share a roadway.
//   • the two Light Rail lines, route_type 0, in the official green (E–W) and
//     blue (S–N), on the rail slice of the graph with the metro treatment.
...
// the minibus taxis — amber, apart from the navy formal buses; they ride the
// props the GZM metrolines used (mline/mstop/mall/msome) and Cairo's
// paratransit reused
...
// ONE feed, TWO cfgs: routeTypes splits the bundle per mode (3 = road
// transport, 0 = light rail). Without the filter `--all` on the road mode
// would swallow the LRT too.
//
// Line keys are the operators' own designations, with ONE normalisation. The
// minibus names carry a redundant "Tx"/"TX" token and a space before the
...
// The only thing in the feed that tells a minibus route from a bus route: the
// suffix AddisMap writes on the long name. Verified against the separately
// published et-addisababa-minibus_2026.zip — the two sets are identical, all
// 251 of them.
const isPara = (r) => /\(Minibus\)\s*$/.test(r.route_long_name || '');
const MODES = [{
...
// The two Light Rail lines get the rail-trunk treatment (wide ribbon, station
// discs, always-on names) — 39 stations on a viaduct, the only fixed-track
// transport in the city.
const isRailTrunk = (l) => l === 'E-W' || l === 'S-N';
// Official colours: the East–West line (Ayat ↔ Tor Hailoch) is green, the
// South–North (Kality ↔ Menelik II Square) blue — a brighter azure than the
// navy of the buses, the trick Cairo's M1 taught us. Where the two share the
// central viaduct the engine paints the mixed set purple (Bucharest's rule),
// not the tram red this family keeps for actual trams.
if (tramAll || tramLines.length) MODES.push({
  mode: 'tram', label: 'light rail', osmFile: 'data/osm/addis-rail.json',
  graphMode: 'tram', railKeep: new Set(['light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramLines,
  feeds: [
    { tag: 'aa', dir: 'data/gtfs', mapKey: lineKey, routeTypes: ['0'],
      // no lineColor: the two Light Rail lines run on street track, so they
      // are trams by the family's reckoning and take its red like every other
      // tram. (They used to carry a hand-picked green and blue.)
    },
  ],
});
...
async function processMode(cfg) {
  ...
  cfg.mlineSet = new Set(); // the minibus taxis — filled from feed.mline below
  cfg.lineColors = {};
  cfg.lineColorsDark = {};
  ...
  for (const feed of cfg.feeds) {
    ...
    if (cfg.mlineSet.size) {
      log(`${feed.tag}: ${cfg.mlineSet.size} minibus-taxi routes (amber), ` +
          `${routeToLine.size - cfg.mlineSet.size} formal bus routes`);
    }
    ...
    // ALSO a TRUNCATED shape. AddisMap extracts these shapes from the OSM route
    // relations, and where a relation carries only its first few ways the shape
    // comes out as a stub: AB010/0 is 10 points and 200 m for an 11 km route,
    // A18/1 is 410 m for 14 km, C15/1 and AB009/0 are empty in all but name.
    // Matched as shapes they produced a 200 m line and then a 12 km "terminal
    // repair" tacked onto its end — the same routing the pseudo path does, only
    // with a false 200 m head start.
    // The bar: a COMPLETE shape can never be shorter than the straight line
    // through its own stops (it goes the same way round, by road). So the ratio
    // is 1 or more for anything healthy — and the feed agrees: 739 of 891 reps
    // land between 1.0 and 1.3, 133 fall below 0.9, and the gap between the two
    // populations is all but empty (8 reps in 0.9–1.0). Cutting at 0.95 puts
    // every truncated shape on the stop-sequence path and leaves the healthy
    // ones alone.
    const STUB_SHARE = 0.95;
    let stubs = 0;
    for (const r of feedReps) {
      if (!r.shapeLatLon || r.shapeLatLon.length < 2 || r.pseudo) continue;
      const st = r.stopSeq.map((x) => stopsById.get(x.stopId)).filter(Boolean);
      if (st.length < 2) continue;
      const mLen = (pts, la = (p) => p[0], lo = (p) => p[1]) => {
        let t = 0;
        for (let i = 1; i < pts.length; i++) {
          const k = 111320 * Math.cos(la(pts[i - 1]) * Math.PI / 180);
          t += Math.hypot((lo(pts[i]) - lo(pts[i - 1])) * k, (la(pts[i]) - la(pts[i - 1])) * 111320);
        }
        return t;
      };
      const chain = mLen(st, (p) => p.lat, (p) => p.lon);
      if (chain > 0 && mLen(r.shapeLatLon) < STUB_SHARE * chain) { r.shapeLatLon = null; stubs++; }
    }
    if (stubs) log(`${stubs} truncated shapes discarded — those reps match on their stop sequence instead`);
    for (const r of feedReps) {
