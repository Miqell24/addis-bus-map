...
export function buildGraph(elements, proj, mode = 'road') {
  ...
  for (const el of elements) {
    ...
    // Addis writes most street names in Latin already (5 128 of 5 294 named
    // ways), but 158 carry Ethiopic script — and the base map's glyph set has
    // no Ethiopic, so those would render as empty boxes. Where OSM offers an
    // English name beside the Ethiopic one, that is the label; the rest keep
    // their own name, which is the honest thing to draw.
    const ETHIOPIC = /[\u1200-\u137F]/;
    const nm = el.tags.name || '';
    const name = (ETHIOPIC.test(nm) && el.tags['name:en']) || nm || el.tags.ref || '';
