// Frontend: MapLibre GL + OpenFreeMap vector tiles (positron) + AddisMap line layers
...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Routes: GTFS AddisMapTransit / DigitalTransport4Africa (ODbL)' }));
  ...
  // minibus membership is by COLOUR, not by name: the association prefixes
  // (ADK, Lafto, Kolfa…) are data, and a name test would have to be kept in
  // step with the feed. The amber is set once, in the pipeline.
  const nM = meta.lines.filter((l) => l.mode === 'bus' && l.color === '#e8a000').length;
  document.getElementById('count').textContent = `(${nBus - nM} bus · ${nM} minibus taxi · ${nTram} light rail)`;
  ...
  // selected + selMode: a line is picked by NAME AND MODE. Here every key in
  // the feed is already unique across all three networks, but the pair costs
  // nothing and keeps the frontend honest if that ever stops being true.
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: AddisMapTransit / DT4A (ODbL)';
      ...
      a.download = `addis-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `addis-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the Sheger buses reach Aleltu, Debre Zeit and
  // Teji, 40–50 km out, and fitting that would open on highland farmland.
  // Open on the city, the ring of condominium suburbs inside the frame.
  map.jumpTo({ center: [38.765, 9.010], zoom: 11.2 });
}
